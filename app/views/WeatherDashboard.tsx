...
export function WeatherDashboard() {
  ...
  const convertForecastData = (
    ...
    convertedData.daily.apparentTemperatureMax = convertedData.daily.apparentTemperatureMax.map(
      (temp: number) => convertTemperature(temp, fromUnit, toUnit)
    );
    
    convertedData.daily.apparentTemperatureMin = convertedData.daily.apparentTemperatureMin.map(
      (temp: number) => convertTemperature(temp, fromUnit, toUnit)
    );
  ...
  const toggleUnit = () => {
    setUnit(prevUnit => {
      ...
      if (currentWeather) {
        const convertedWeather = convertWeatherData(currentWeather, prevUnit, newUnit);
        setCurrentWeather(convertedWeather);
      }
      
      if (forecast) {
        const convertedForecast = convertForecastData(forecast, prevUnit, newUnit);
